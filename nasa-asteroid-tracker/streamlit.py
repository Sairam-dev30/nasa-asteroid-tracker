...
def main():
    ...
    col3.metric("Avg Diameter (km)", f"{(df['estimated_diameter_max_km'].mean() + df['estimated_diameter_min_km'].mean())/2:.4f}")
    ...
    "Top 5 Largest Asteroids": """
        SELECT a.neo_reference_id, a.name, a.estimated_diameter_max_km 
        FROM asteroids a 
        ORDER BY a.estimated_diameter_max_km DESC 
        LIMIT 5
    """,
    "Top 5 Fastest Asteroids": """
        SELECT a.neo_reference_id, a.name, c.relative_velocity_kmph 
        FROM asteroids a
        JOIN close_approach c ON a.id = c.neo_reference_id
    ...
    "Potentially Hazardous Asteroids": """
        SELECT a.neo_reference_id, a.name, a.is_potentially_hazardous_asteroid 
        FROM asteroids a 
        WHERE a.is_potentially_hazardous_asteroid = 1
    """,
    "Asteroids Closest to Earth (Min Astronomical Unit)": """
        SELECT a.neo_reference_id, a.name, c.astronomical
        FROM asteroids a
        JOIN close_approach c ON a.id = c.neo_reference_id
